...
<body class="min-h-screen flex flex-col">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex-shrink-0 flex items-center">
                    ...
                    <button id="accessToggleButton" class="hidden sm:flex items-center text-sm font-medium text-gray-500 ml-2 pl-2 border-l border-gray-300 group" data-mode="viewer">
    ...
    <main class="flex-grow">
        <section class="hero-section py-20 sm:py-32">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                <a href="#feature-blocks" id="heroCtaLink" class="inline-flex items-center justify-center bg-white text-primary-dark font-bold text-base px-7 py-3 rounded-full shadow-xl hover:bg-gray-100 transition duration-300 transform hover:scale-105">
                    View Public Reports
            ...
            <div id="footerGrid" class="grid grid-cols-2 md:grid-cols-4 gap-8 pb-8 border-b border-white/20 mb-6">
                ...
                <div>
                    ...
                    <p class="text-xs mt-2 opacity-75">
                        Post-Disaster Recovery & Information Management System. Streamlining Recovery, From Your Barangay 
                    </p>
                </div>
    ...
    <script>
        ...
        function getSignupStep1FormModal(isMember) {
            const title = isMember ? 'Purok Member Registration (1 of 2)' : 'General Viewer Registration (1 of 2)';
            ...
            const memberFields = isMember ? `
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-1 mb-4"><label for="contact" class="block text-sm font-medium text-gray-700 mb-1">Contact Number</label><input type="tel" id="contact" name="contact" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition" placeholder="e.g., 09123456789" pattern="[0-9]*" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '')" required value="${getVal('contact')}"></div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                     <div class="col-span-1 mb-4"><label for="age" class="block text-sm font-medium text-gray-700 mb-1">Age</label><input type="number" id="age" name="age" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition" min="1" max="120" required value="${getVal('age')}"></div>
            ...
            return `
                ...
                <form onsubmit="handleStep2Submit(event, ${isMember})">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <input type="password" id="password" name="password" class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg ${focusRing} transition" placeholder="••••••••" required>
                            <button type="button" onclick="togglePasswordVisibility('password', 'signupPasswordIcon')" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600">
                                <i id="signupPasswordIcon" data-lucide="eye" class="w-5 h-5"></i>
                            </button>
                        </div>
                    </div>
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            <input type="password" id="confirmPassword" name="confirmPassword" class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg ${focusRing} transition" placeholder="••••••••" required>
                            <button type="button" onclick="togglePasswordVisibility('confirmPassword', 'confirmPasswordIcon')" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600">
                ...
                return `
                    ...
                    <form onsubmit="handleAuthSubmit(event, 'Login')">
                        ...
                        <div class="mb-6">
                            <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Password</label>
                            <div class="relative">
                                <input type="password" id="password" class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg ${focusRing} transition" placeholder="••••••••" required>
                                <button type="button" onclick="togglePasswordVisibility('password', 'loginPasswordIcon')" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600">
                                    <i id="loginPasswordIcon" data-lucide="eye" class="w-5 h-5"></i>
                                </button>
                            </div>
                        </div>
                        <button type="submit" class="w-full ${buttonBg} text-white font-semibold py-2.5 rounded-lg shadow-md transition duration-150">
            ...
            return `
                ...
                <form onsubmit="handleManagementSubmit(event)">
                    <div class="mb-4">
                        ...
                        <input type="text" id="officialId" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-aid-color focus:border-aid-color transition" placeholder="e.g., 12345678@email.com" required>
                    </div>
                    <div class="mb-4">
                        <label for="officialPin" class="block text-sm font-medium text-gray-700 mb-1">Official Password / PIN</label>
                        <div class="relative">
                            <input type="password" id="officialPin" class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:ring-aid-color focus:border-aid-color transition" placeholder="••••••••" required>
                            <button type="button" onclick="togglePasswordVisibility('officialPin', 'officialPinIcon')" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600">
                                <i id="officialPinIcon" data-lucide="eye" class="w-5 h-5"></i>
                            </button>
                        </div>
                    </div>
                ...
                <form onsubmit="handleRequestSubmit(event)">
                    <div class="grid grid-cols-3 gap-2 mb-4">
                        <div class="col-span-1">
                            ...
                            <input type="text" id="firstName" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition" required>
                        </div>
                        <div class="col-span-1">
                            <label for="middleInitial" class="block text-sm font-medium text-gray-700 mb-1">M.I.</label>
                            <input type="text" id="middleInitial" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition">
                        </div>
                        <div class="col-span-1">
                            <label for="surname" class="block text-sm font-medium text-gray-700 mb-1">Surname</label>
                            <input type="text" id="surname" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition" required>
                        </div>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-5">
                        <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                        <input type="text" id="email" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition" required>
                    </div>
                    <div class="mb-6">
                        <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Password / PIN</label>
                        <input type="password" id="password" class="w-full px-3 py-2 border border-gray-300 rounded-lg ${focusRing} transition" required>
                    </div>
                    <button type="submit" class="w-full ${buttonBg} text-white font-semibold py-2.5 rounded-lg shadow-md transition duration-150">
                        Submit Request
                    </button>
        ...
        function getPlaceholderModal(action) {
            let title, message;
            const buttonBg = currentAccessMode === 'management' ? 'bg-mgm-dark hover:bg-mgm-light' : 'bg-primary-dark hover:bg-primary-light';
            const highlightColor = currentAccessMode === 'management' ? 'text-mgm-light' : 'text-primary-light';
            const highlightBorder = currentAccessMode === 'management' ? 'border-mgm-light' : 'border-primary-light';
            const highlightText = currentAccessMode === 'management' ? 'text-mgm-dark' : 'text-primary-dark';
            ...
            return `
                <h2 class="text-2xl font-bold ${highlightColor} mb-2">${title}</h2>
                <p class="text-sm text-gray-500 mb-4">Action Placeholder</p>
                <div class="p-4 bg-gray-100 rounded-lg border-l-4 ${highlightBorder}">
                    <p class="text-gray-700">${message}</p>
                    <p class="mt-2 text-xs font-semibold ${highlightText}">NOTE: The backend is currently not connected.</p>
                </div>
                <button onclick="hideModal()" class="mt-6 w-full ${buttonBg} text-white font-semibold py-2 rounded-lg transition">
                    Got It
                </button>
            `;
        }
        ...
        function handleFormSubmit(event, formName) {
            ...
            // specific handling for login forms
            if (formName.includes('Authentication') || formName.includes('Management ID Verification')) {
                ...
                if (!username || !password) {
                    alertMessage("Please enter both username/ID and password.", "error");
                    return;
                }

                loginUser(username, password)
                    .then(data => {
                        // Check which form was used to enforce role restrictions
                        if (isManagement) {
                            // Official/Admin Login Flow
                             if (data.role === 'admin') {
                                ...
                                alertMessage("Access Denied: This portal is for Barangay Officials only.", "error");
                            }
                        } else {
                            // Viewer Access Portal Flow (Top Right Login)
                            if (data.role === 'viewer' || data.type === 'viewer' || (!data.role && !data.originalRole)) {
                                 // Viewer Login
                                 localStorage.setItem('currentUser', JSON.stringify(data));
                                 window.location.href = 'viewer.php';
                            } else {
                                // Admin/Official trying to log in via Viewer Portal
                                alertMessage("Access Denied: This login is for Community Residents only. Please use the Management Portal.", "error");
                            }
                        }
                    })
                    .catch(err => {
            ...
            if (formName === 'Account Request for Admin Approval') {
                const form = event.target;
                const firstName = document.getElementById('firstName').value;
                const middleName = document.getElementById('middleName')?.value || document.getElementById('middleInitial')?.value || ''; 
                // Handle different dom structures if reused, but here we know the ids
                const surname = document.getElementById('surname').value;
                const email = document.getElementById('email').value;
                const role = document.getElementById('role').value;
                const password = document.getElementById('password').value;

                fetch('api_request_official.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                         firstName, middleName, surname, email, role, password
                    })
                })
                .then(res => res.json())
                .then(data => {
                    ...
                    hideModal();
                    alertMessage(data.message || "Account request submitted.", "info");
                })
                .catch(err => alertMessage(err.message, "error"));
                return;
            }

            // Fallback for other forms (like signup, which is handled by step2 submit, or requests)
            hideModal();
        ...
        function handleStep2Submit(event, isMember) {
            ...
            const finalUserData = {
                ...
                isMember: isMember,
                purok: isMember ? signupData.purok : null,
                isHead: isMember ? signupData.isHead : false,
                householdHead: isMember ? signupData.householdHead : null
            };

            fetch('api_signup_viewer.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            .then(data => {
                ...
                showModal(`
                    ...
                    <p class="text-sm text-gray-500 mb-4">Welcome to PDRIMS.</p>
                    <div class="p-4 bg-green-50 rounded-lg border-l-4 border-green-500 mb-6">
                        <p class="text-green-800">Your account has been created successfully. You can now log in to access the system.</p>
                    </div>
                    <button onclick="toggleAuthModal('Login')" class="w-full bg-primary-dark text-white font-semibold py-2.5 rounded-lg shadow-md hover:bg-primary-light transition duration-150">
                        Proceed to Login
                    </button>
                `);
            })
