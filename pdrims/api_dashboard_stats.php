...
try {
    ...
    // 4. Damage Stats (Breakdown by Damage Status)
    $damageStats = [];
    $stmt = $pdo->query("SELECT damage_status, COUNT(*) as count FROM households GROUP BY damage_status");
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        // Ensure key is string for consistent JSON
        $damageStats[(string)$row['damage_status']] = $row['count'];
    }
